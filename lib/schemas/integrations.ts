...
/**
 * Clé OpenAI **directe** (pas OpenRouter) — utilisée pour la
 * transcription audio Whisper. `""` = supprimer, sinon doit commencer
 * par `sk-` (et explicitement PAS par `sk-or-` qui est OpenRouter).
 */
export const updateOpenAiKeySchema = z.object({
  apiKey: z
    ...
    .max(300, "Clé trop longue.")
    .refine(
      (v) => v === "" || (/^sk-[A-Za-z0-9_-]{20,}$/.test(v) && !v.startsWith("sk-or-")),
      "Format OpenAI attendu : `sk-…` (pas `sk-or-…` qui est OpenRouter).",
    ),
});
