...
type SearchParams = Promise<{ range?: Range; week?: string }>;
...
export default async function TempsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const { start, end, label } = resolveRange(activeRange, week);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Reporting"
        title="Temps"
        description={`Réalisé vs planifié — ${label}.`}
      />

      <div className="flex flex-wrap items-center gap-2">
        <FilterLink href="/temps?range=week" active={activeRange === "week"} label="Semaine" />
        <FilterLink href="/temps?range=month" active={activeRange === "month"} label="Mois" />
        <FilterLink href="/temps?range=all" active={activeRange === "all"} label="Tout" />

        {prevWeek && nextWeek ? (
          <div className="ml-auto flex items-center gap-1">
            <Button variant="outline" size="sm" asChild>
              <Link href={`/temps?range=week&week=${isoDate(prevWeek)}`}>← Préc.</Link>
            </Button>
            <Button variant="outline" size="sm" asChild>
              <Link href="/temps?range=week">Cette semaine</Link>
            </Button>
            <Button variant="outline" size="sm" asChild>
              <Link href={`/temps?range=week&week=${isoDate(nextWeek)}`}>Suiv. →</Link>
            </Button>
          </div>
        ) : null}
      </div>
